// Jedi.swift

import Foundation

// The below type is accessible to Objective-C via its `@objc` attribute,
// `public` access, and inheritance of `NSObject` (via `ForceSensitiveBeing`).
//
// Note: All Objective-C compatible Swift types are exposed outside of the
// module as they require `open` or `public` visibility in order to be
// Objective-C compatible in the first place. This means that all superclasses
// must also be `public` or `open`. In this case, `ForceSensitiveBeing` is
// a public header.
@objc public class Jedi: ForceSensitiveBeing {
    @objc public let name: String

    @objc public init(name: String) {
        self.name = name
    }

    // Returns `true` if this Jedi wins against the given Sith; otherwise, `false`.
    // Note that the `Sith` is declared by the Objective-C half of the target.
    @objc public func duel(sith: Sith) -> Bool {
        // Only Master Yoda can defeat the Sith.
        if self.name == "Yoda" {
            // We print using the C function defined in droid_debug.h.
            DROID_DEBUG(
                "Master Jedi \(self.name) defeated Sith Lord \(sith.name)"
            );
            return true
        } else {
            // We print using the C function defined in droid_debug.h.
            DROID_DEBUG(
                "Sith Lord \(sith.name) defeated Master Jedi \(self.name)"
            );
            return false
        }
    }

    // Access target's resource bundle.
    @objc public func readAncientJediTexts() throws -> String {
        // The `Bundle.module` is generated by SwiftPM to provide access to the
        // target's bundled resources.
        let path = Bundle.module.path(forResource: "hello_there", ofType: "txt")
        let contents = try String(contentsOfFile: path!, encoding: .utf8)
        return contents
    }
}
